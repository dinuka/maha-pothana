...
async def count_section_translation_progress(
    ...
    """Return pageId -> (section_count, translated_count, approved_count, rejected_count).
    ...
    rejected_count: sections where every submitted translation is rejected and
    none is approved (used for the REJECTED review-status override — mirrors
    the re-entry check in sections.py's _check_reentry_logic). A section that
    was rejected but later resolved via an approved (or freshly resubmitted,
    not-yet-rejected) translation no longer counts, even though old rejected
